...
test.describe('View detail page', () => {
    ...
    test('create view from selection', async ({ appUri, moxy, page }) => {
        ...
        const columnPostLogs = moxyLog.filter(log => log.path == `/v1/orgs/1/people/views/${NewView.id}/columns` && log.method == 'POST');
        const viewPostLogs = moxyLog.filter(log => log.path == '/v1/orgs/1/people/views' && log.method == 'POST');
        const rowPutLogs = moxyLog.filter(log => log.path.startsWith(`/v1/orgs/1/people/views/${NewView.id}/rows/`) && log.method == 'PUT');
        ...
        expect(page.url()).toEqual(appUri + `/organize/1/people/views/${NewView.id}`);
    });
});
