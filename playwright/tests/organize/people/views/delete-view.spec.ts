...
const expectDeleteViewSuccess = (moxy: NextWorkerFixtures['moxy']) => {
    const deleteViewRequest = moxy.log().find((mock) =>
        mock.method === 'DELETE' &&
...
test.describe('Delete view from view list page', () => {
    ...
    test('user can delete a view from a menu in the list', async ({ page, appUri, moxy }) => {
        ...
        moxy.setZetkinApiMock(`/orgs/1/people/1/views/${AllMembers.id}`, 'delete', undefined, 204);
    ...
    test('shows an error if view deletion fails', async ({ page, appUri, moxy }) => {
        moxy.setZetkinApiMock('/orgs/1/people/views', 'get',  [AllMembers]);
        moxy.setZetkinApiMock(`/orgs/1/people/1/views/${AllMembers.id}`, 'delete', undefined, 405);
