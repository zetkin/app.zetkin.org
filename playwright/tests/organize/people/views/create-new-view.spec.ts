...
test.describe('Views list page', () => {
    ...
    test(
        ...
        async ({ page, appUri, moxy }) => {
            ...
            moxy.setZetkinApiMock(`/orgs/1/people/views/${NewView.id}/columns`, 'get', NewViewColumns);
            ...
            const columnPostLogs = moxy.log<{title: string}>()
                .filter(log => log.path === `/v1/orgs/1/people/views/${NewView.id}/columns` && log.method === 'POST');
            const viewPostLogs = moxy.log<{title: string}>()
                .filter(log => log.path === '/v1/orgs/1/people/views' && log.method === 'POST');
