...
test.describe('Creating a view column', () => {
    ...
    test.beforeEach(({ moxy, login }) => {
        ...
        moxy.setZetkinApiMock('/orgs/1/people/views/1/columns', 'get',  [AllMembersColumns[0], AllMembersColumns[1]]);
    });
    ...
    // Mock for post req to create new column
        moxy.setZetkinApiMock('/orgs/1/people/views/1/columns', 'post',AllMembersColumns[2], 201);
        ...
        const columnPostReq = moxy.log().find(mock => mock.method === 'POST' && mock.path === '/v1/orgs/1/people/views/1/columns');
    ...
    test('shows an error modal if there is an error creating the column', async ({ page, appUri, moxy }) => {
        moxy.setZetkinApiMock('/orgs/1/people/views/1/columns', 'post', undefined, 404);
