...
const SurveyLayout: React.FC<SurveyLayoutProps> = ({
  ...
  const handleDelete = async () => {
    await deleteSurvey();
    await router.push(
      `/organize/${orgId}/projects/${surveyFuture.data?.project?.id || ''} `
    );
  };
  ...
  const handleOnProjectSelected = async (projectId: number) => {
    const updatedSurvey = await updateSurvey({ project_id: projectId });
    await router.push(
    ...
    showSnackbar(
      ...
      messages.surveyChangeProjectDialog.success({
        projectTitle: updatedSurvey.project!.title,
  ...
  return (
    <TabbedLayout
      ...
      onClickAlertBtn={() => {
        router.push(
          `/organize/${originalOrgId}/projects/${surveyFuture.data?.project?.id}/surveys/${surveyId}`
        );
      }}
      subtitle={
