...
const surveysSlice = createSlice({
  ...
  reducers: {
    ...
    surveyCreated: (state, action: PayloadAction<ZetkinSurveyExtended>) => {
      ...
      if (survey.project) {
        if (!state.surveyIdsByProjectId[survey.project.id]) {
          state.surveyIdsByProjectId[survey.project.id] = remoteList();
        }
        state.surveyIdsByProjectId[survey.project.id].items.push(
