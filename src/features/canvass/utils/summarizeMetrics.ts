  ...
  return {
    metrics: Object.values(responsesByMetricId).map((responses) => {
      ...
      if (isBool) {
        ...
        const toNumericResponse = (response: MetricResponse) =>
          Number(response.response);

        return {
          metric_id: responses[0].metric_id,
          num_values: [
            responses.filter((resp) => toNumericResponse(resp) === 1).length,
            responses.filter((resp) => toNumericResponse(resp) === 2).length,
            responses.filter((resp) => toNumericResponse(resp) === 3).length,
            responses.filter((resp) => toNumericResponse(resp) === 4).length,
            responses.filter((resp) => toNumericResponse(resp) === 5).length,
          ],
        };
      }
    }),
