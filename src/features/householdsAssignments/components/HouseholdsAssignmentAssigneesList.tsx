...
const HouseholdsAssignmentAssigneesList = ({
  assignees,
  onCustomize,
  onRemove,
}: {
  assignees: ZetkinHouseholdsAssignee[];
  onCustomize: (assignee: ZetkinHouseholdsAssignee) => void;
  onRemove: (assignee: ZetkinHouseholdsAssignee) => void;
}) => {
  const messages = useMessages(messageIds);
  const { orgId } = useRouter().query;

  const columns: GridColDef<ZetkinHouseholdsAssignee>[] = [
    {
    ...
    },
    {
      field: 'name',
      flex: 1,
      headerName: messages.assignees.nameColumn(),
      valueGetter: (params) =>
    ...
    },
    {
      ...
      flex: 1,
      headerName: messages.assignees.prioritizedTagsColumn(),
      renderCell: (props) => {
    ...
    },
    {
      field: 'excluded_tags',
      flex: 1,
      headerName: messages.assignees.excludedTagsColumn(),
      renderCell: (props) => {
        ...
        return (
          <Box>
            <Button onClick={() => onCustomize(props.row)} variant="outlined">
              <Msg id={messageIds.assignees.customizeButton} />
            </Button>
            <ZUIEllipsisMenu
              items={[
                {
                  label: messages.assignees.actions.customize(),
                  onSelect: () => onCustomize(props.row),
                  startIcon: <Edit />,
                },
                {
                  label: messages.assignees.actions.remove(),
...
export default HouseholdsAssignmentAssigneesList;
