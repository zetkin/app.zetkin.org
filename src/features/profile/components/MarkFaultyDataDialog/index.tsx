...
import EditPersonFields from './MarkPersonFields';
...
const EditPersonDialog: FC<EditPersonDialogProps> = ({
  ...
  const [markedFieldValues, setMarkedFieldValues] = useState<string[]>([]);
      ...
      >
        ...
        <Box overflow="auto" width="100%">
          <EditPersonFields
            ...
            markedFields={markedFieldValues} 
            onChange={(field, newValue) => {
              onFieldValueChange(field, newValue?.trim() ?? null);
              if (newValue === 'faulty') {
                setMarkedFieldValues([...markedFieldValues, field]);
              } else {
                setMarkedFieldValues(
                  markedFieldValues.filter((f) => f !== field)
                );
              }
            }}
            onReset={(field) => {
