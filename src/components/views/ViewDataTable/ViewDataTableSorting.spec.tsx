...
describe('ViewDataTableSorting.tsx', () => {
    ...
    const gridColumns = fields.map((field, idx) => {
        return  makeGridColDef(mockViewCol({
            config: { field },
            id: idx,
            title: field,
    ...
    const sortModel: GridSortModel = gridColumns.slice(0,2).map(column => ({ field: column.field, sort: 'asc' }));

    const handleSetSortModel = jest.fn(sortModel => sortModel);

    it('renders correctly when sort model is empty', async () => {
        const { getByText } = render(
            <ViewDataTableSorting gridColumns={ gridColumns } setSortModel={ () => null } sortModel={ [] } />,
        );
    ...
    it('renders correctly when sort model is populated', async () => {
        const { getAllByText, getByText } = render(
            <ViewDataTableSorting gridColumns={ gridColumns } setSortModel={ handleSetSortModel } sortModel={ sortModel } />,
        );
