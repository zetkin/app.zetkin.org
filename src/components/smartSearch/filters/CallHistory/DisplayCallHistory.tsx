...
import getCallAssignment from 'fetching/getCallAssignment';
import { getTimeFrameWithConfig } from '../../utils';
import { CallHistoryFilterConfig, OPERATION, SmartSearchFilterWithId } from 'types/smartSearch';

interface DisplayCallHistoryProps {
    filter: SmartSearchFilterWithId<CallHistoryFilterConfig>;
}

const DisplayCallHistory = ({ filter }: DisplayCallHistoryProps) : JSX.Element => {
    ...
    const {
        ...
        numDays,
        timeFrame,
    } = getTimeFrameWithConfig({ after: config.after, before: config.before });
        ...
        <Msg
            ...
            values={{
                ...
                assignmentSelect: assignmentTitle ?
                    <Msg id="misc.smartSearch.call_history.assignmentSelect.assignment"
                        values={{
                            assignmentTitle,
                        }}
                    /> :
                    <Msg id="misc.smartSearch.call_history.assignmentSelect.any"/>,
                callSelect: <Msg id={ `misc.smartSearch.call_history.callSelect.${operator}` }/>,
                minTimes: minTimes || 1,
