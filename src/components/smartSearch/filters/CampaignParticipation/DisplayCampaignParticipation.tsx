...
import getActivity from 'fetching/getActivity';
import getCampaign from 'fetching/getCampaign';
import getLocation from 'fetching/getLocation';
import { getTimeFrameWithConfig } from '../../utils';
import { CampaignParticipationConfig, OPERATION, SmartSearchFilterWithId } from 'types/smartSearch';

interface DisplayCampaignParticipationProps {
    filter: SmartSearchFilterWithId<CampaignParticipationConfig>;
}

const DisplayCampaignParticipation = ({ filter }: DisplayCampaignParticipationProps) : JSX.Element => {
    const { orgId } = useRouter().query;
    const { config } = filter;
    const { operator, state, campaign: campId, activity: activityId, location: locationId } = config;
    ...
    return (
        <Msg
            ...
            values={{
                activitySelect: activityTitle ? <Msg id="misc.smartSearch.campaign_participation.activitySelect.activity" values={{
                    activity: activityTitle,
                }}
                /> : <Msg id="misc.smartSearch.campaign_participation.activitySelect.any"/>,
                addRemoveSelect: (
                    <Msg id={ `misc.smartSearch.campaign_participation.addRemoveSelect.${op}` }/>
                ),
                bookedSelect: (
                    <Msg id={ `misc.smartSearch.campaign_participation.bookedSelect.${state}` }/>
                ),
                campaignSelect: campaignTitle ? <Msg id="misc.smartSearch.campaign_participation.campaignSelect.campaign" values={{
                    campaign: campaignTitle,
                }}
                /> :<Msg id="misc.smartSearch.campaign_participation.campaignSelect.any"/>,
                haveSelect: (
                    <Msg id={ `misc.smartSearch.campaign_participation.haveSelect.${operator}` }/>
                ),
                locationSelect: locationTitle ? <Msg id="misc.smartSearch.campaign_participation.locationSelect.location" values={{
