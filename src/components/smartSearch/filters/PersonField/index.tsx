...
import { CUSTOM_FIELD_TYPE } from 'types/zetkin';
...
import {  NewSmartSearchFilter, OPERATION, PersonFieldFilterConfig,
...
interface PersonFieldProps {
    ...
    onSubmit: (filter: SmartSearchFilterWithId<PersonFieldFilterConfig> |
        ZetkinSmartSearchFilter<PersonFieldFilterConfig>) => void;
    onCancel: () => void;
}

const DEFAULT_VALUE = 'none';

const PersonField = ({ onSubmit, onCancel, filter: initialFilter }: PersonFieldProps): JSX.Element => {
    const { orgId } = useRouter().query;
    const fieldsQuery = useQuery(['customFields', orgId], getCustomFields(orgId as string));
    const fields = fieldsQuery.data || [];

    const filteredFields = fields.filter(f => f.type !== CUSTOM_FIELD_TYPE.JSON);

    const { filter, setConfig, setOp } = useSmartSearchFilter<Partial<PersonFieldFilterConfig>>(
        initialFilter);
    ...
    const submittable = selectedField &&
    ...
    return (
        <FilterForm
            ...
            renderSentence={ () => (
                <Msg id="misc.smartSearch.person_field.inputString" values={{
                    addRemoveSelect: (
                        <StyledSelect onChange={ e => setOp(e.target.value as OPERATION) }
                            value={ filter.op }>
                            { Object.values(OPERATION).map(o => (
                                <MenuItem key={ o } value={ o }>
                                    <Msg id={ `misc.smartSearch.person_field.addRemoveSelect.${o}` }/>
                                </MenuItem>
                            )) }
                        </StyledSelect>
                    ),
                    field: (
                        <Msg
                            id={ `misc.smartSearch.field.edit.${selectedField?.type || DEFAULT_VALUE}` }
                            values={{
                                fieldSelect: selectedField ? (
                                    ...
                                    <StyledSelect
                                        SelectProps={{ renderValue: function getLabel() {
                                            return (
                                                <Msg
                                                    id="misc.smartSearch.field.fieldSelect.any"
                                                />);
                                        } }}
                                        value={ DEFAULT_VALUE }>
